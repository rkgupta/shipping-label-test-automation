...
import com.pb.testautomation.label.engine.LabelComparatorEngine;
...
{
	Logger logger = Logger.getLogger(PDFImageWriter.class);
    /**
    ...
    throws IOException
    {
        ...
        for( int i = startPage - 1; i < endPage && i < pages.size(); i++ )
        {
            ...
            try
            {
                ...
                //Check mode here. Do the masking only if the mode of operation is comparison mode.
                LabelComparatorEngine engine = LabelComparatorEngine.getInstance();
                if(engine != null && engine.isInComparisonMode()) {
                	BufferedImage maskedImage = MaskingUtil.getInstance().applyMaskOnGeneratedImg( new File( fileName ) , image);
                	if(maskedImage != null) {
                		image = maskedImage;
                	} else {
                		//This is the case when the configuration is not found in the config file for this page
                		//(but there is config for this type of pdf). Let the image page be unmasked in this case.
                		
                	}                	
                }
                logger.info(Messages.getMessage(CommonConstants.INFO_WRITING_IMAGE, fileName));
