...
/**
 * Represents the handler class that generates the test report in a file.
 * @author RA013GU
 *
 */
public class ComparisonReportHandler extends ReportHandler {

	private static int labelNo = 1;
	private int matchingLabelsCount = 0;
	private Logger logger = Logger.getLogger(ComparisonReportHandler.class);
	private HSSFWorkbook workbook;
	private HSSFSheet sheet;
	
	/**
	 * Creates a FileReportHandler instance.
	 * @param labelComparator
	 */
	public ComparisonReportHandler(File master, File test){
		super(master, test);
		labelNo = 1;
	...
	private String getReportFileHeader() {
		StringBuilder reportBuilder = new StringBuilder();
		reportBuilder.append("Comparison result for labels "+master.getAbsolutePath()
				+" and "+test.getAbsolutePath()+" \ndated "
	...
	/**
	 * Constructs the Test execution summary file header.
	 * @return
	 * @throws IOException 
	...
	private void writeStatusFileHeader() throws IOException {
		...
		//Master Folder
		HSSFRow masterFolderRow = sheet.createRow(8);
		
		HSSFCell masterFolderCell = masterFolderRow.createCell(0);
		masterFolderCell.setCellValue(new HSSFRichTextString("MASTER_FOLDER = "));
		
		HSSFCell masterFolderVal = masterFolderRow.createCell(1);
		masterFolderVal.setCellValue(new HSSFRichTextString(master.getAbsolutePath()));
		
		
		//Test Folder
		HSSFRow testFolderRow = sheet.createRow(9);
		...
		//Diff Image
		HSSFRow diffImageRow = sheet.createRow(10);
		...
		HSSFRow headersRow = sheet.createRow(12);
	...
	/**
	 * Generates the label comparison report and writes it into the <code>reportFile</code>.
	 */
	public File generateReport() {
		FileOutputStream fos = null;
		StringBuilder reportBuilder = new StringBuilder();
		reportBuilder.append("\nTime taken to compare the labels: "+CommonUtil.getExecutionTimeFormat(executionTime));
		reportBuilder.append("\n-------------------------------------END------------------------------------\n");
		appendToFile(reportFile, reportBuilder.toString());
		
		try {
			int row = 12 + labelNo + 2;
